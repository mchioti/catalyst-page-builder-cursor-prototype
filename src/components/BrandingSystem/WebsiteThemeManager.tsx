...
interface WebsiteThemeManagerProps {
  websiteId: string;
  websiteName: string;
}

export const WebsiteThemeManager: React.FC<WebsiteThemeManagerProps> = ({ 
  websiteId, 
  websiteName 
}) => {
  const {
  ...
  // Initialize branding for this website if it doesn't exist
  React.useEffect(() => {
    if (!getWebsiteBranding(websiteId)) {
      initializeWebsiteBranding(websiteId);
    }
    setActiveWebsite(websiteId);
  }, [websiteId, getWebsiteBranding, initializeWebsiteBranding, setActiveWebsite]);
  ...
  const handleItemEdit = (item: SubjectBranding | JournalBranding | BookSeriesBranding) => {
    if (themeManager.activeTab === 'subjects') {
      updateSubject(websiteId, item.id, item);
    } else if (themeManager.activeTab === 'journals') {
      updateJournal(websiteId, item.id, item);
    } else if (themeManager.activeTab === 'bookSeries') {
      updateBookSeries(websiteId, item.id, item);
    }
    setEditingItem(null);
  };
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
          ...
          {websiteName} Base Branding
        </h3>
        <p className="text-sm text-gray-600 mb-4">
          These colors serve as the fallback branding when no specific subject, journal, or series branding applies for this website.
        </p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(websiteBranding.website.colors).map(([key, value]) => (
  ...
  const renderItemsTab = (type: 'subjects' | 'journals' | 'bookSeries') => {
    const items = websiteBranding[type];
    const labels = {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Manage branding for {type === 'subjects' ? 'subject categories' : type} on {websiteName}
            </p>
          </div>
  ...
  if (themeManager.editingItem) {
    if (themeManager.editingItem === 'website') {
      ...
      return (
        <div className="p-6">
          <div className="bg-white p-6 rounded-lg border border-gray-200">
            <h3 className="text-lg font-semibold mb-4">Edit {websiteName} Branding</h3>
    ...
    // Edit existing item
    const activeItems = websiteBranding[themeManager.activeTab === 'subjects' ? 'subjects' : 
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="border-b bg-white px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{websiteName} Branding</h1>
            <p className="text-sm text-gray-600">
              Manage website-specific branding for subjects, journals, and book series
            </p>
          </div>
        </div>
      </div>
